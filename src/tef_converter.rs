...
pub fn handle_partial_system_call(syscall_store: &mut Vec<SystemCall>,call: Option<SystemCall>,counter: &mut usize) -> Option<SystemCall>{

    let call_ref = call.as_ref().expect("System call to handle should exist");

    if call.is_some(){

        if call_ref.call_type == CallType::UnfinishedCall{
            syscall_store.push(call.clone()?); 
            return call;
        }
        else if call_ref.call_type == CallType::ResumedCallWithArgs || call_ref.call_type == CallType::ResumedCallWithoutArgs || call_ref.call_type == CallType::ResumedWithoutDur{
...
// is found
fn check_storage(syscall_store: &mut Vec<SystemCall>,resumed_call: &Option<SystemCall>,counter: &mut usize) -> Option<SystemCall>{

    let resumed_call_ref = resumed_call.as_ref().unwrap();
    ...
    for (i,stored_syscall) in syscall_store.iter_mut().enumerate()
    {
        ...
        if stored_syscall.pid == resumed_call_ref.pid &&
            ...
            let merged_call = Some(stored_syscall.clone());
...
pub fn build_trace_event_format(event: &Option<SystemCall>)-> TEFSystemCall<'_>{

    match event.as_ref().unwrap().call_type {
